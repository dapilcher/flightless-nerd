	...
	render() {
		...
		return (
			<Fragment>
				...
				<div
					className="filters__form__wrapper"
				...
				>
					<form id="filters__form">
						{categories
							.filter(cat1 =>
								posts.some(post =>
									post.categories.some(cat2 => cat1._id === cat2._id)
								)
							)
							.map(cat => (
								<Checkbox
				...
				<ArticleCardGrid
					count={0}
					posts={
						Object.values(this.state.filters).some(val => val)
							? posts.filter(post =>
									post.categories.some(cat => this.state.filters[cat._id])
							  )
							: posts
					}
				/>
